...
class GitterTestCheckout(unittest.TestCase):
    def test_checkout_local_branch(self):
        ...
        gt.run()
        self.assertEquals(GIT + "checkout develop", gt._last_command)

    def test_remote_branch(self):
        gt = TestingGitRunner(["checkout"])
        gt._selected = [gitter.RemoteBranch('bugfix')]
        gt.run()
        self.assertEquals(GIT + "checkout -b bugfix bugfix", gt._last_command)

    def test_modified(self):
        gt = TestingGitRunner(["checkout"])
        gt._selected = [gitter.ModifiedFile('somefile.py')]
        gt.run()
        self.assertEquals(GIT + "checkout somefile.py", gt._last_command)

    def test_deleted(self):
        gt = TestingGitRunner(["checkout"])
        gt._selected = [gitter.DeletedFile('deleted.py')]
        gt.run()
        self.assertEquals(GIT + "checkout deleted.py", gt._last_command)

    def test_multiple(self):
        gt = TestingGitRunner(["checkout"])
        gt._selected = [gitter.ModifiedFile('somefile.py'), gitter.DeletedFile('deleted.py')]
        gt.run()
        self.assertEquals(GIT + "checkout somefile.py deleted.py", gt._last_command)
